...
function MyBodyHome() {
  ...
  const getPostByPlace = async () => {
    try {
      ...
      const { data } = await axios.get(`/common/post/place?name=${_place}&page=${pagingNumber}`);
              ...
              >
                <Box sx={style} style={{ borderRadius: 8 }}>
                  <Stack direction='column' justifyContent='center' textAlign='center' spacing={1}>
                    <Stack direction='row' justifyContent='center' spacing={2} alignItems='center' style={{ backgroundColor: '#7b35ba', borderRadius: 4 }}>
                      <LocationCityIcon fontSize='large' style={{ fill: 'white' }} />
                      <p style={{ fontSize: 40, color: 'white' }}>where are you living?</p>
                    </Stack>
                    <Divider />
                    {
                      listPlace.map(item => {
                        return (
                          <div key={item.id}>
                            <Stack direction='row' justifyContent='center' spacing={2} alignItems='center' style={{ borderRadius: 4 }}>
                              <p onClick={() => {
                                console.log('ha noi');
                                localStorage.setItem('suggest_place', item.place)
                                getPostByPlace()
                                handleClose()
                              }} style={{ fontSize: 40, cursor: 'pointer' }}>{item.place}</p>
                            </Stack>
                            <Divider />
                          </div>
                        )
